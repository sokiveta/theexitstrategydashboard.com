...
if ($user_id > 0 && is_array($task_ids) && $employee_id > 0 && $demo == 'FALSE') {
  foreach ($task_ids as $task_id) {              
    $taskSQL = "UPDATE `tasks` SET `employee_id`='".$employee_id."' WHERE `task_id`=".$task_id." AND `user_id`=".$user_id;
